...
- **版本：** V0.8.0（雛型階段；版本號自 V0.2.0 重新定基）
...
V0.8.0（雛型）：功能完整的獨立事審送審準備工具，顯示名「彰濱秀傳癌症中心 癌藥事審輔助系統」。條文卡手風琴收合（開一關一、預設展開第一張）縮短長清單；免疫藥型態→單用/併用兩層分組；移除冗餘的「需事前審查」提醒框。流程癌別優先（個管各管自己的癌別、不混申請）：選癌別→選藥→核對條文→備齊送審。蘋果風桌機優先等寬雙欄工作台、手機響應收合、鍵盤可及性齊備；密碼 cbshow。版本自成一軌、不對齊母程式。下版第一優先是「下次母程式改版時實走資料更新流程」。
