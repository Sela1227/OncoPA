...
- **版本：** V0.39.3（資料日期 115.7.23；HER2 比照 PD-L1／ALK 歸蛋白質層級）
...
65. **「檢測機構」欄填的是認證體系，不是機構名稱——而原本的預設值是錯的**
   - 背景：NHOS3001 基因資訊分頁的「檢測機構」選取視窗含 CAP／TAF／TFDA／TSP 四個認證／列管體系（亦有院所名稱可選）。原內建 Prompt 寫「院內常態選 CAP」，V0.39.0 附錄B 甚至寫「選本院（彰濱秀傳病理科分子病理實驗室）」——後者在只有認證別的情境下根本填不下去。
   - 院內回饋（病理分子組與 NGS 單位分別答覆）：**本院病理科分子病理實驗室＝TAF**（檢驗醫學部通過 TAF ISO 15189，認證編號 2202）、**本院 NGS＝TFDA**、**外院報告依該報告載明之認證或機構填寫**（常見 CAP）。同一家醫院會有兩個以上答案，因為這欄跟著「執行該檢測的實驗室」跑，不是跟著醫院跑。
   - **判準要分兩層，不可只留排除法**：第一判準是報告表頭的執行單位名稱；「HIS 病理報告文字檔抓得到＝病理分子、須到抽血報告 PDF 才看得到＝NGS」只是輔助線索。抓不到文字檔的原因不只 NGS（外院報告、尚未上傳、掃描檔皆可能），僅憑「抓不到」就判 NGS 會把外院病理分子報告誤填成 TFDA。
   - **推測值必須標示**：依輔助線索推得者，AI Prompt 輸出時加「★推測（依據：…）→ 請確認」，由個管師／NP 核對報告表頭後定案；不可讓推測值長得像已確認值。
   - 三處必須同步（Rule 14）：①病摘 AI Prompt（輸出樣板、範例、填寫規則第 3 條、外院報告連動、內部自我核對第④項）②操作精靈 S11 操作與核對、附錄B ③送審備件清單的認證別提示區塊。只改精靈不改 Prompt，AI 會繼續吐 CAP。
   - 條文關聯：通則十二要求認證實驗室、9.37 Bevacizumab 指定情境另需伴隨式診斷 30104B——此欄正是該項佐證，填錯不只是欄位錯，會與報告載明的認證對不上。

66. **不是每個生物標記都屬「基因資訊」——蛋白質層級的要走檢附報告**
   - 病理科分子組回覆：**PD-L1 與 ALK 屬蛋白質層級**（本院皆以免疫染色判讀），不登錄 NHOS3001 基因資訊分頁，直接檢附病理報告。原 `GENE_LOINC` 的 PD-L1 條目甚至寫著「SOP：PD-L1 走免疫染色仍可填基因資訊」，與此直接相反。
   - 分辨原則：**看檢測層級，不是看標記名稱**。基因層級（點突變、融合／重排、擴增、MSI）→ 填基因資訊；蛋白質層級（IHC 表現量或表現判讀）→ 檢附病理報告。ALK 之所以反直覺，是因為它在文獻上是融合基因，但本院實際以 IHC 判讀，故歸蛋白質。
   - 例外要留門：外院若送來 FISH／NGS 的 ALK 重排或 EML4-ALK 融合報告，那是基因層級——已標「★需洽病理科確認是否登錄」，原 SOP 代碼保留為「暫不使用」而非刪除，日後決議恢復時可直接查閱。**不要因為改規則就把知識刪掉。**
   - 既有機制可用：`GENE_LOINC` 早有 `report` 欄（渲染「附病理報告」標籤，RET／HRD／Claudin 18.2／MSI-MMR／17p／IGHV／CD 抗原都是），把 ALK、PD-L1 改成 `report` 型即完成備件清單側的行為改變，不需新增程式路徑。
   - 三處同步（Rule 14）：①`GENE_LOINC` ②病摘 AI Prompt（最高原則新增「蛋白質層級不進基因檢測明細」、輸出樣板新增「不填基因資訊」行、方法對照、代碼對照表兩列、ALK 細表、PD-L1 碼清單）③操作精靈（S11 常見錯誤、S13 核對與常見錯誤、附錄A 三處、附錄B 補註）。**只改精靈不改 Prompt，AI 會繼續把 PD-L1／ALK 塞進基因明細。**
   - HER2 於 V0.39.3 經 Sela 確認比照辦理，三項一致。三項的例外門（ALK 之 FISH／NGS 重排、HER2 之 FISH／ISH 擴增或 ERBB2/CEP17 比值）皆標「★需洽病理科確認是否登錄」，不讓個管自行填碼。
   - 附帶教訓：新增編號條目時要檢查既有編號。V0.39.2 在 Prompt【最高原則】插入第 4 條，造成清單出現兩個「4.」，V0.39.3 才修正（4→5、5→6）。字串插入式改法不會有人幫你重新編號。
...
| 版本 | 重點 |
|------|------|
| V0.39.3 | **HER2 比照 PD-L1／ALK 歸蛋白質層級**，不填基因資訊。`GENE_LOINC` 之 HER2 改為 `report` 型（原院內候選碼 49683-6／31150-6／42783-1／74860-8／74885-5／85318-4／85328-3／96893-3 保留標為暫不使用）；例外門：FISH／ISH 擴增或 ERBB2/CEP17 比值報告屬基因層級，標「★需洽病理科確認是否登錄」，不自行填碼。同步 Prompt 五處（最高原則第 4 條、檢測方法對照、HER2 選碼規則、代碼對照表 HER2 列、HER2 碼清單）與精靈三處（S11 常見錯誤、附錄A 不填清單、附錄B 補註）。**併修** V0.39.2 遺留缺失：Prompt【最高原則】因插入新條目而出現兩個「4.」，已重編為 1–6。**驗收**：GENE_LOINC 三項（PD-L1／ALK／HER2）皆為 report 型、EGFR 等基因層級不受影響；9.18 乳癌與 9.69 肺癌備件清單均出現「附病理報告」標籤；Prompt 舊「HER2 須先判斷檢測方法再選碼」已移除、新規則到位、最高原則編號 1–6 連續；精靈 15 步＋2 附錄三區上限仍符合；條文功能回歸不受影響（290 卡、716/86/705）。⚠ 坑 #66 |
| V0.39.2 | **PD-L1、ALK 改歸蛋白質層級，不填基因資訊**（病理科分子組回覆：兩項為免疫染色之蛋白質表現，不登錄 NHOS3001 基因資訊分頁，直接檢附病理報告）。原 `GENE_LOINC` 之 PD-L1 條目寫「PD-L1 走免疫染色仍可填基因資訊」為錯誤，已更正。**做法**：兩者改為既有的 `report` 型（渲染「附病理報告」標籤），不新增程式路徑；ALK 原 SOP 代碼（78205-2／78210-2／78233-4／88744-8 等）保留標為「暫不使用」而非刪除，並留外院 FISH／NGS 之 ALK 重排／EML4-ALK 融合例外門，標「★需洽病理科確認是否登錄」。**三處同步**：①`GENE_LOINC` ②病摘 AI Prompt（最高原則新增第 4 條「蛋白質層級項目不進基因檢測明細」、輸出樣板新增「不填基因資訊（改檢附病理報告）」行、檢測方法對照、代碼對照表 PD-L1／ALK 兩列、ALK 細表、PD-L1 碼清單）③操作精靈（S11 常見錯誤由「可登錄基因資訊分頁」反轉為「不登錄」、S13 核對與常見錯誤、附錄A 變異型態／融合類／新增不填清單、附錄B 補註）。**驗收**：jsdom 確認 GENE_LOINC 之 ALK／PD-L1 均為 report 型（HER2／NTRK 不受影響）、9.69 肺癌備件清單出現「附病理報告」標籤、Prompt 舊敘述已移除且兩條新規則到位；精靈 15 步＋2 附錄三區上限仍符合；條文功能回歸不受影響（290 卡、716/86/705）。**待確認**：HER2 IHC 是否比照辦理（分子組未點名，維持原狀並於 S11 標示）。⚠ 坑 #66 |
| V0.39.1 | **基因報告「檢測機構」欄定義修正**（依病理分子組與 NGS 單位回饋）。該欄語意為「執行該檢測之實驗室所屬的認證／列管體系」：**本院病理科分子病理實驗室＝TAF**（檢驗醫學部 ISO 15189，認證編號 2202）、**本院 NGS＝TFDA**、**外院報告依該報告載明之認證或機構填寫**（常見 CAP）。原內建 Prompt 的「院內常態選 CAP」與 V0.39.0 附錄B 的「選本院（彰濱秀傳病理科分子病理實驗室）」皆為錯誤，已更正。判準改兩層：第一判準為報告表頭執行單位名稱；「病理報告文字檔抓得到＝病理分子／須到抽血報告 PDF＝NGS」僅為輔助線索，且輸出時須標「★推測（依據：…）→ 請確認」由個管師／NP 核對後定案（抓不到文字檔亦可能是外院報告或尚未上傳，不得逕判 NGS）。**三處同步**：①病摘 AI Prompt（輸出樣板、範例、填寫規則第 3 條、外院報告連動、內部自我核對第④項）②操作精靈 S11 操作＋核對、附錄B ③送審備件清單新增認證別提示區塊（`.prep-lab`，僅在備件含基因／生物標記項目時顯示）。**驗收**：jsdom 確認 9.99 顯示認證別區塊且含 TAF／TFDA／CAP／2202／推測字樣、9.55（無基因備件）不顯示；Prompt 三處舊敘述全數移除、三條新規則到位；精靈 15 步＋2 附錄三區上限仍符合；條文功能回歸不受影響（290 卡、716/86/705）。⚠ 坑 #65 |
...
7. **內建 Prompt 範例用 21666-3 EGFR（"mutations tested for"）當病人結果** — 與精靈附錄A、`GENE_LOINC` 的 21665-5（mutations found）語意衝突。`GENE_LOINC` 註明「SOP 有明訂者以 SOP 為準」，故未逕改；需與病理科確認 SOP 究竟指定哪一碼後統一三處（坑 #7 同源問題）
8. **HIS〈適應症〉欄位長度單位待實貼驗證**（已確認先以 200 字為政策） — 若為 varchar(200) 位元組（Big5 中文 2 位元組＝約 100 中文字），現行濃縮版（中位 148 字）仍會被截斷，需再壓一輪至約 100 字。請個管師實貼一筆最長的（如 9.85 卵巢維持 199 字）驗證；畫面已即時顯示字數便於判斷（坑 #56）
9. **`gene-marker-report.html` 內嵌 DRUGS 為 remap 前舊快照** — 本輪同步 `geneDocs`／`markerDocs` 兩函式時查出：報表內嵌的是 **103 筆、癌別仍為「肝膽胰胃癌／血液淋巴癌／泌尿癌／婦癌／頭頸癌／其他癌」的 V0.25.0 前快照**（現行為 105 筆、19 癌別）。函式已同步，資料未同步 → 報表逐(藥×癌別)結果與現行工具不一致。建議把「由 index.html 重生報表」寫成 `tools/` 腳本，納入資料更新 SOP
10. **雙向 diff 工具化（坑 #47）** — 現行 diff 以新版為基準，對「官方刪短」與「items 落後 per_cancer」不敏感；建議把這兩種比對寫進 `tools/`，接在資料更新 SOP 之後
