...
- **版本：** V0.14.0（雛型階段；版本號自 V0.2.0 重新定基）
...
V0.14.0（雛型）：功能完整的獨立事審送審準備工具。手機 RWD 完成（斷詞防溢出＋小螢幕斷點）；通用規定（含擇一/互換、劑量、無效後不給付、使用注意事項）（事前審查/劑量/通則/療程上限）與劑型標記不再列為可選適應症，改入共通規定區/分隔；收合標題萃取僅供顯示、不混入複製；共通規定可逐條件打勾，顯示名「彰濱秀傳癌症中心 癌藥事審輔助系統」。條文卡手風琴收合；選癌別頁垂直置中、精簡冗語；已全藥稽核並拆分「Erbitux 型態」多適應症條文（Cetuximab、Bevacizumab、Rituximab、Lenalidomide、Osimertinib），病人對應其一。流程癌別優先（個管各管自己的癌別、不混申請）：選癌別→選藥→核對條文→備齊送審。蘋果風桌機優先等寬雙欄工作台、手機響應收合、鍵盤可及性齊備；密碼 cbshow。版本自成一軌、不對齊母程式。下版第一優先是「下次母程式改版時實走資料更新流程」。
