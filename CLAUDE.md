...
- **版本：** V0.37.0（資料日期 115.7.23；送審備件清單三層分流＋條文正確性修正）
...
7. **`gene-marker-report.html` 內嵌 DRUGS 為 remap 前舊快照** — 本輪同步 `geneDocs`／`markerDocs` 兩函式時查出：報表內嵌的是 **103 筆、癌別仍為「肝膽胰胃癌／血液淋巴癌／泌尿癌／婦癌／頭頸癌／其他癌」的 V0.25.0 前快照**（現行為 105 筆、19 癌別）。函式已同步，資料未同步 → 報表逐(藥×癌別)結果與現行工具不一致。建議把「由 index.html 重生報表」寫成 `tools/` 腳本，納入資料更新 SOP
8. **雙向 diff 工具化（坑 #47）** — 現行 diff 以新版為基準，對「官方刪短」與「items 落後 per_cancer」不敏感；建議把這兩種比對寫進 `tools/`，接在資料更新 SOP 之後
