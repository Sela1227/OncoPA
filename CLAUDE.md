...
- **版本：** V0.38.0（資料日期 115.7.23；進入碼遮蔽切換＋適應症 200 字欄位濃縮複製）
...
7. **確認 HIS〈適應症〉欄是 200 字還是 200 位元組** — 若為 varchar(200) 位元組（Big5 中文 2 位元組＝約 100 中文字），現行濃縮版（中位 148 字）仍會被截斷，需再壓一輪至約 100 字。請個管師實貼一筆最長的（如 9.85 卵巢維持 199 字）驗證；畫面已即時顯示字數便於判斷（坑 #56）
